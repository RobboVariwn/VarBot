...
public class LanguageManager : MonoBehaviour, ISerializationCallbackReceiver
{
	...
	void Awake ()
	{
		...
		if(LoadAvailableCultures())
		{
			...
			{
				SmartCultureInfo defaultLanguageCulture = availableLanguages.cultureInfos.Find(IsSystemLanguage);
			
				var enLanguageCulture = availableLanguages.cultureInfos?.Find(x=>x.languageCode.Equals("en"));

				if (enLanguageCulture != null)
				{
					defaultLanguageCulture = enLanguageCulture;
				}
				...
				{	
					Debug.LogError("cannot load default language config");
