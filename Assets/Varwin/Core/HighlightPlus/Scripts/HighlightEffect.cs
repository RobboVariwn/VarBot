...
namespace HighlightPlus
{
	...
	public partial class HighlightEffect : MonoBehaviour
	{
		...
		static int[] mipGlowBuffers, mipOutlineBuffers;
		static Mesh quadMesh;
		int sourceRT, glowRT, outlineRT;
		...
		void OnRenderObject ()
		{
...
#if UNITY_EDITOR
			bool glowOnTop = useSmoothGlow && (!glowOptimalBlit || glowAlwaysOnTop || "SceneCamera".Equals (cam.name));
			bool outlineOnTop = useSmoothOutline && (!outlineOptimalBlit || outlineAlwaysOnTop || "SceneCamera".Equals (cam.name));
#else
			...
			if (useSmoothBlend) {
				...
				if (glowOnTop || outlineOnTop || "SceneCamera".Equals (cam.name)) {
		...
		bool ComputeSmoothQuadMatrix (Camera cam, Bounds bounds)
		{
			...
			if (glow > 0) {
				BuildMatrix (cam, scrMin, scrMax, (int)(50 + 50 * glowWidth), ref quadGlowMatrix);
			}
			return true;
		}
				...
				if (rms [k].mesh != null) {
					...
					for (int l = 0; l < rms [k].mesh.subMeshCount; l++) {
						...
						if (fxMat != null) {
							...
							if (_highlighted && (glowAlwaysOnTop || outlineAlwaysOnTop)) {
								fxMat.SetInt ("_ZTest", (int)UnityEngine.Rendering.CompareFunction.Always);
							} else {
								fxMat.SetInt ("_ZTest", (int)UnityEngine.Rendering.CompareFunction.LessEqual);
							}
						}
