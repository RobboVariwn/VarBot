...
namespace ZenFulcrum.EmbeddedBrowser
{
    ...
    public class VRBrowserHand : MonoBehaviour
    {
        ...
        /// </summary>
        public MouseButton DepressedButtons { get; set; }
        /// <summary>
        ...
        protected virtual void ReadInput()
        {
            ...
            //var leftClick = VRInput.GetAxis(nodeState, InputAxis.TouchPad);
...
Set to a negative number to enable that infernal ""natural scrolling"" that's been making so many trackpads unusable lately.")]
        ...
        private void UpdatePreCull(Camera cam)
        {
            ...
            for (int i = 0; i < states.Count; i++)
            {
                ...
                nodeState = states[i];
                Tracked = nodeState.tracked;

                Vector3 pos;
                if (states[i].TryGetPosition(out pos))
                {
                    transform.position = pos + Player.position;
                }
                Quaternion rot;
                if (states[i].TryGetRotation(out rot))
                {
                    transform.localRotation = rot;
                }

                if (visualization)
                    visualization.SetActive(Tracked);
            }
        }

        public void SelectPrefabToSpawn()
        {
