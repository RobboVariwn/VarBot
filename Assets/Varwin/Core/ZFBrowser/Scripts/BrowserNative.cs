            ...
            set
            {
                if (NativeLoaded)
                    throw new InvalidOperationException(
                        "ProfilePath must be set before initializing the browser system.");
                _profilePath = value;
            }
        }
...
#if UNITY_STANDALONE_WIN && !UNITY_EDITOR && !VRMAKER
...
//			throw new DllNotFoundException("ZFBrowser failed to load " + name + ": " + Marshal.GetLastWin32Error());
                throw new DllNotFoundException("ZFBrowser failed to load " +
                                               name +
                                               ": " +
		...
		*/
		...
		var success = dlclose(moduleHandle) == 0;
            ...
            if (!success)
            {
                throw new DllNotFoundException("Failed to unload library: " +
                                               GetLibError());
            }
        ...
        private static IntPtr GetFunc(IntPtr libHandle, string fnName)
        {
            ...
            if (addr == IntPtr.Zero)
            {
                throw new DllNotFoundException("ZFBrowser failed to load method " +
                                               fnName +
                                               ": " +
        ...
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        public delegate void ConsoleFunc(int browserId, string message, string source,
            int line);
         ...
         */
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        public delegate void ForwardJSCallFunc(int browserId, int callbackId, string data,
            int size);
         ...
         */
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        public delegate void ShowContextMenuFunc(int browserId, string menuJSON, int x,
            int y, ContextMenuOrigin origin);
         ...
         */
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        public delegate void NavStateFunc(int browserId, bool canGoBack, bool canGoForward,
            bool lodaing, IntPtr url);
         ...
         */
        public delegate IntPtr Calltype_zfb_flatColorTexture(int pixelCount, int r, int g,
            int b, int a);
        ...
        public delegate void Calltype_zfb_mouseButton(int id, MouseButton button, bool down,
            int clickCount);
         ...
         */
        public delegate void Calltype_zfb_getMouseCustomCursor(int id, IntPtr buffer, int width,
            int height, out int hotX, out int hotY);
        ...
        public delegate void Calltype_zfb_sendDialogResults(int id, bool affirmed, string text1,
            string text2);
        ...
        public delegate void Calltype_zfb_registerPopupCallback(int id, NewWindowAction windowAction,
            ZFBSettings baseSettings, NewWindowFunc cb);
         ...
         */
        public delegate void Calltype_zfb_downloadCommand(int id, int downloadId, DownloadAction command,
            string fileName);
