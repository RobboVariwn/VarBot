...
namespace ZenFulcrum.EmbeddedBrowser
{
    ...
    public class VRMainControlPanel : MonoBehaviour
    {
        ...
        public Transform BroserTransform;
        ...
        public List<VRBrowserPanel> allBrowsers = new List<VRBrowserPanel>();
        ...
        private void ShiftTabs(int direction)
        {
            allBrowsers = allBrowsers
                .Select((t, i) => allBrowsers[(i + direction + allBrowsers.Count) % allBrowsers.Count]).ToList();
        }
        ...
        public void MoveToward(Transform t, Vector3 scale)
        {
        ...
        public void Update()
        {
            ...
            for (int i = 0; i < allBrowsers.Count; i++)
            {
                ...
                MoveToward(allBrowsers[i].transform, Vector3.one);
            }

            {
                var pos = keyboardTarget ? keyboardTarget.keyboardLocation.position : Vector3.zero;
                var rot = keyboardTarget
                    ? keyboardTarget.keyboardLocation.rotation
                    : Quaternion.LookRotation(Vector3.down, Vector3.forward);
                var scale = keyboardTarget ? baseKeyboardScale : Vector3.zero;
                MoveToward(keyboard.transform, scale);
            }
